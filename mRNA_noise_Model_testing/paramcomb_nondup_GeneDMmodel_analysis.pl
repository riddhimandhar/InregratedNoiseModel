...
$model[13]="Model14 DM_YPD~Model9+Model11-13";
$model[14]="Model9 DM_YPD~TATAbox+PromNucOcc+Histone_Mods+TF_binding+mRNAprop+PTM";  
$model[15]="Model14 DM_YPD~Model9+Model11-13+PTM";
...
if($dtc==0) 
{ 
   print "Unfiltered nondup Data ...$model[$modt]\n";  
   $R->run(q'tab=tab[,which(colSums(is.na(tab))<2000)]');
   $R->run(q'tab_filt1 <- tab[,c(4,7:246)]');
   ...
   if($modt==2) { $R->run(q'inputData=tab_filt2[,c(1,171:222)]'); } ## Histone_mods
   if($modt==3) { $R->run(q'inputData=tab_filt2[,c(1,3:32,171:222)]'); }  ##TATAbox + PromNucOcc + Histone_mods
   if($modt==4) 
   { 
     print WR "---------------------------------------------------------\n";
   ...
   if($modt==5) { $R->run(q'inputData=tab_filt2[,c(1,33:161)]'); }   ## TF_binding
   if($modt==6) { $R->run(q'inputData=tab_filt2[,c(1,3:161,171:222)]'); }  ## TATAbox+PromNucOcc+Histone_mods+TF_binding 
   if($modt==7) { $R->run(q'inputData=tab_filt2[,c(1,3:161,171:222,227)]'); }  ## TATAbox+PromNucOcc+Histone_mods+TF_binding+mRNAprop
   ...
   if($modt==14) { $R->run(q'inputData=tab_filt2[,c(1,3:161,171:222,227:241)]'); }  ## TATAbox+PromNucOcc+Histone_mods+TF_binding+mRNAprop+PTM
    ##print "$name $pval\n"; 
}
if($dtc==1)
{
   print "CorrFiltered nondup Data ...$model[$modt]\n";
   $R->run(q'tab_filt1 <- tab[,c(4:165)]');
   $R->run(q'tab_filt2 <- tab_filt1[,which(apply(tab_filt1, 2, var, na.rm=TRUE) != 0)]');
   ...
   if($modt==2) { $R->run(q'inputData=tab_filt2[,c(1,106:136)]'); } ## Histone_mods
   if($modt==3) { $R->run(q'inputData=tab_filt2[,c(1,3:27,106:136)]'); }  ##TATAbox + PromNucOcc + Histone_mods
   ...
   if($modt==5) { $R->run(q'inputData=tab_filt2[,c(1,28:96)]'); }   ## TF_binding
   if($modt==6) { $R->run(q'inputData=tab_filt2[,c(1,3:96,106:136)]'); }  ## TATAbox+PromNucOcc+Histone_mods+TF_binding
   if($modt==7) { $R->run(q'inputData=tab_filt2[,c(1,3:96,106:136,138:145)]'); }  ## TATAbox+PromNucOcc+Histone_mods+TF_binding+mRNAprop
   if($modt==13) { $R->run(q'inputData=tab_filt2[,c(1,3:96,106:136,138:155)]'); }  ## Combinations 
   if($modt==14) { $R->run(q'inputData=tab_filt2[,c(1,3:96,106:136,138:145,156:162)]'); }  ## TATAbox+PromNucOcc+Histone_mods+TF_binding+mRNAprop+PTM
   if($modt==15) { $R->run(q'inputData=tab_filt2[,c(1,3:96,106:136,138:162)]'); }  ## Combinations+PTM
}
if($dtc==2) ###
{
   print "ImpFiltered nondup Data ...$model[$modt]\n";
   $R->run(q'tab_filt1 <- tab[,c(4,7:31)]');
   $R->run(q'tab_filt2 <- tab_filt1[,which(apply(tab_filt1, 2, var, na.rm=TRUE) != 0)]');
   if($modt>=0 && $modt<=4)
   {
     print WR "---------------------------------------------------------\n";
   ...
   if($modt==5) { $R->run(q'inputData=tab_filt2[,c(1,3:23)]'); }   ## TF_binding
   if($modt==6) { $R->run(q'inputData=tab_filt2[,c(1:24)]'); }  ## TATAbox+PromNucOcc+Histone_mods+TF_binding
   ...
   if($modt==13) { $R->run(q'inputData=tab_filt2[,c(1:26)]'); }  ## Combinations 
}
