...
$R->run(q'write.table(inputData,"UNFILTGENE")');

$cn=0; 
open(FP,"UNFILTGENE") or die;
while($fp=<FP>)
{
  chomp($fp); 
  if($cn==0)
...
system "rm UNFILTGENE"; 

$wrfile="IndParam_impact_".$iter."_Iterations.txt"; 
...
open(GR,">Results/Impactful_parameter_list_DM.txt") or die; 
