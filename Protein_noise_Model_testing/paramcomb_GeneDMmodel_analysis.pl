...
{ 
   ...
   $R->run(q'tab_filt1 <- tab[,c(4,7:266)]');
   ...
   if($modt==8 || $modt==9 || $modt==10 || $modt==11 || $modt==12) ## 
   {
      print WR "---------------------------------------------------------\n";
      print WR2 "---\t---\t---\t---\t---\t---\t";
      print WR3 "---\t---\t---\t---\t---\t---\t";
    ...
    if($modt==13) { $R->run(q'inputData=tab_filt2[,c(1,3:151,161:212,217:246)]'); }  ## Combinations
    ...
    if($modt==15) { $R->run(q'inputData=tab_filt2[,c(1,3:151,161:212,217:260)]'); }  ## modt7+modt8-11+PTM  ##Except Metal  
    ##print "$name $pval\n"; 
}
if($dtc==1)
{
   print "CorrFiltered Data ...$model[$modt]\n";
   $R->run(q'tab_filt1 <- tab[,c(4:143)]'); ## Except Metal 
   $R->run(q'tab_filt2 <- tab_filt1[,which(apply(tab_filt1, 2, var, na.rm=TRUE) != 0)]');
   ...
   if($modt==2) { $R->run(q'inputData=tab_filt2[,c(1,98:124)]'); } ## Histone_mods
   if($modt==3) { $R->run(q'inputData=tab_filt2[,c(1,3:21,98:124)]'); }  ##TATAbox + PromNucOcc + Histone_mods
   ...
   if($modt==5) { $R->run(q'inputData=tab_filt2[,c(1,22:89)]'); }   ## TF_binding
   if($modt==6) { $R->run(q'inputData=tab_filt2[,c(1,3:89,98:124)]'); }  ## TATAbox+PromNucOcc+Histone_mods+TF_binding
   if($modt==7) { $R->run(q'inputData=tab_filt2[,c(1,3:89,98:126)]'); }  ## TATAbox+PromNucOcc+Histone_mods+TF_binding+mRNAprop
   if($modt==13) { $R->run(q'inputData=tab_filt2[,c(1,3:89,98:135)]'); }  ## Combinations
   if($modt==14) { $R->run(q'inputData=tab_filt2[,c(1,3:89,98:126,136:140)]'); }  ## TATAbox+PromNucOcc+Histone_mods+TF_binding+mRNAprop+PTM
   if($modt==15) { $R->run(q'inputData=tab_filt2[,c(1,3:89,98:140)]'); }  ## Combinations+PTM
}
if($dtc==2)
{
   print "ImpFiltered Data ...$model[$modt]\n";
   $R->run(q'tab_filt1 <- tab[,c(4,7:39)]');
   $R->run(q'tab_filt2 <- tab_filt1[,which(apply(tab_filt1, 2, var, na.rm=TRUE) != 0)]');
   if($modt>=0 && $modt<=4)
   {
     print WR "---------------------------------------------------------\n";
   ...
   if($modt==5) { $R->run(q'inputData=tab_filt2[,c(1,4:28)]'); }   ## TF_binding
   if($modt==6) { $R->run(q'inputData=tab_filt2[,c(1,3:31)]'); }  ## TATAbox+PromNucOcc+Histone_mods+TF_binding
   if($modt==7) { $R->run(q'inputData=tab_filt2[,c(1,3:32)]'); }  ## TATAbox+PromNucOcc+Histone_mods+TF_binding+mRNAprop
   if(($modt>=8 && $modt<=12) || ($modt==14) || ($modt==15))
   {
     print WR "---------------------------------------------------------\n";
     print WR2 "---\t---\t---\t---\t---\t---\t";
     print WR3 "---\t---\t---\t---\t---\t---\t";
   ...
   if($modt==13) { $R->run(q'inputData=tab_filt2[,c(1,3:34)]'); }   ## Combination 
}
