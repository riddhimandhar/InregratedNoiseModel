...
$datatype[0]="Filtered_nondup_MERGED_DATA_Yeast.txt"; 
$datatype[1]="CorrFiltered_nondup_MERGED_DATA_Yeast.txt"; 
$datatype[2]="ImpFiltered_nondup_MERGED_DATA_Yeast.txt"; 
...
$wrfile="SubModel_ResultsGeneDM_".$iter."_Iterations_nondup.txt";
$wrfile2="Formatted_frvarExpl_SubModelGeneDM_".$iter."_Iterations_nondup.txt";
$wrfile3="Formatted_predRsq_SubModelGeneDM_".$iter."_Iterations_nondup.txt";
...
print WR3 "Model_no\tModel\tMean_prRsq_U_Ridge\tMean_prRsq_U_RidgeAIC\tMean_prRsq_U_Lasso\tMean_prRsq_U_LassoAIC\tMean_prRsq_U_RF_wi\tMean_prRsq_U_RF_wo\t";
print WR3 "Mean_prRsq_F_Ridge\tMean_prRsq_F_RidgeAIC\tMean_prRsq_F_Lasso\tMean_prRsq_F_LassoAIC\tMean_prRsq_F_RF_wi\tMean_prRsq_F_RF_wo\t";
...
$model[12]="Model14 DM_YPD~Model10-13";
$model[13]="Model15 DM_YPD~Model9+Model10-13";
$model[14]="Model16 DM_YPD~TATAbox+PromNucOcc+Histone_Mods+TF_binding+mRNAprop+PTM";  
$model[15]="Model17 DM_YPD~Model9+Model10-13+PTM";
...
{ 
   ...
   $R->run(q'tab=tab[,which(colSums(is.na(tab))<800)]');
   $R->run(q'tab_filt1 <- tab[,c(4,7:265)]');
   ...
   if($modt==2) { $R->run(q'inputData=tab_filt2[,c(1,161:212)]'); } ## Histone_mods
   if($modt==3) { $R->run(q'inputData=tab_filt2[,c(1,3:32,161:212)]'); }  ##TATAbox + PromNucOcc + Histone_mods
   if($modt==4) 
   { 
     print WR "---------------------------------------------------------\n";
     print WR2 "---\t---\t---\t---\t---\t---\t";
   ...
   if($modt==5) { $R->run(q'inputData=tab_filt2[,c(1,33:151)]'); }   ## TF_binding
   if($modt==6) { $R->run(q'inputData=tab_filt2[,c(1,3:151,161:212)]'); }  ## TATAbox+PromNucOcc+Histone_mods+TF_binding 
   if($modt==7) { $R->run(q'inputData=tab_filt2[,c(1,3:151,161:212,217:228)]'); }  ## TATAbox+PromNucOcc+Histone_mods+TF_binding+mRNAprop
   if($modt==8 || $modt==9 || $modt==10 || $modt==11) ## 
   {
      print WR "---------------------------------------------------------\n";
      print WR2 "---\t---\t---\t---\t---\t---\t";
      print WR3 "---\t---\t---\t---\t---\t---\t";
    ...
    if($modt==12) { $R->run(q'inputData=tab_filt2[,c(1,229:246)]'); }  ## modt7+modt8-11   
    if($modt==13) { $R->run(q'inputData=tab_filt2[,c(1,3:151,161:212,217:246)]'); }  ## modt7+modt8-11   
    if($modt==14) { $R->run(q'inputData=tab_filt2[,c(1,3:151,161:212,217:228,247:260)]'); }  ## TATAbox+PromNucOcc+Histone_mods+TF_binding+mRNAprop+PTM
    if($modt==15) { $R->run(q'inputData=tab_filt2[,c(1,3:151,161:212,217:260)]'); }  ## modt7+modt8-11+PTM 
    ##print "$name $pval\n"; 
}
if($dtc==1)
{
   print "CorrFiltered nondup Data ...$model[$modt]\n";
   $R->run(q'tab_filt1 <- tab[,c(4:137)]');
   $R->run(q'tab_filt2 <- tab_filt1[,which(apply(tab_filt1, 2, var, na.rm=TRUE) != 0)]');
   if($modt==0) { $R->run(q'inputData=tab_filt2[,c(1,4:21)]'); } ## PromNucOcc
   if($modt==1) { $R->run(q'inputData=tab_filt2[,c(1,3:21)]'); } ## TATAbox + PromNucOcc
   if($modt==2) { $R->run(q'inputData=tab_filt2[,c(1,95:120)]'); } ## Histone_mods
   if($modt==3) { $R->run(q'inputData=tab_filt2[,c(1,3:21,95:120)]'); }  ##TATAbox + PromNucOcc + Histone_mods
   ...
   if($modt==5) { $R->run(q'inputData=tab_filt2[,c(1,22:86)]'); }   ## TF_binding
   if($modt==6) { $R->run(q'inputData=tab_filt2[,c(1,3:86,95:120)]'); }  ## TATAbox+PromNucOcc+Histone_mods+TF_binding
   if($modt==7) { $R->run(q'inputData=tab_filt2[,c(1,3:86,95:122)]'); }  ## TATAbox+PromNucOcc+Histone_mods+TF_binding+mRNAprop
   if($modt==13) { $R->run(q'inputData=tab_filt2[,c(1,3:86,95:129)]'); }  ## Combinations
   if($modt==14) { $R->run(q'inputData=tab_filt2[,c(1,3:86,95:122,130:134)]'); }  ## TATAbox+PromNucOcc+Histone_mods+TF_binding+mRNAprop+PTM
   if($modt==15) { $R->run(q'inputData=tab_filt2[,c(1,3:86,95:134)]'); }  ## Combinations+PTM
}
if($dtc==2)
{
   print "ImpFiltered nondup Data ...$model[$modt]\n";
   $R->run(q'tab_filt1 <- tab[,c(4,7:40)]');
   $R->run(q'tab_filt2 <- tab_filt1[,which(apply(tab_filt1, 2, var, na.rm=TRUE) != 0)]');
   if($modt>=0 && $modt<=4)
   {
     print WR "---------------------------------------------------------\n";
   ...
   if($modt==5) { $R->run(q'inputData=tab_filt2[,c(1,6:28)]'); }   ## TF_binding
   if($modt==6) { $R->run(q'inputData=tab_filt2[,c(1:31)]'); }  ## TATAbox+PromNucOcc+Histone_mods+TF_binding
   if($modt==7) { $R->run(q'inputData=tab_filt2[,c(1:32)]'); }  ## TATAbox+PromNucOcc+Histone_mods+TF_binding+mRNAprop
   if($modt>=8 && $modt<=12)
   {
     print WR "---------------------------------------------------------\n";
     print WR2 "---\t---\t---\t---\t---\t---\t";
   ...
   if($modt==13) { $R->run(q'inputData=tab_filt2[,c(1:34)]'); }   ## Combination 
   if($modt==14) { $R->run(q'inputData=tab_filt2[,c(1:32,35)]'); }  ## TATAbox+PromNucOcc+Histone_mods+TF_binding+mRNAprop+PTM
   if($modt==15) { $R->run(q'inputData=tab_filt2[,c(1:35)]'); }   ## Combination+PTM
}


$R->run(q'write.table(inputData,"TMP_nondup.txt")');

open(TM,"TMP_nondup.txt") or die; 
open(TR,">inputData_nondup.txt") or die; 
$cc=0; 
...
system "rm TMP_nondup.txt"; 

$R->run(q'rm(inputData)');
$R->run(q'inputData=read.table("inputData_nondup.txt",header=T)');

system "rm inputData_nondup.txt"; 
...
for($i=0;$i<$iter;$i++)
{
  ...
  $R->run(q'pred=predict(rf,newdata=testData,na.action="na.impute")');
  ...
  if(($modt==13 && $dtc!=0)||($modt==15)) { $R->run(q'pred=predict(rf,newdata=testData,na.action="na.impute")'); } ##
  else
  {
...
if($dtc==0) { print WR "Unfiltered nondup data $iter iterations\n------------------------------------------------------------------------------------------------------\n"; }


if($dtc==1) { print WR "Filtered nondup data $iter iterations\n------------------------------------------------------------------------------------------------------\n"; }
if($dtc==2) { print WR "Filtered2 nondup data $iter iterations\n------------------------------------------------------------------------------------------------------\n"; }
